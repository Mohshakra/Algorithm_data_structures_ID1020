    ...
    public static void main(String[] args) throws IOException {
        ...
        while (!StdIn.isEmpty()) {
            ...
            if (key.length() < minlen)
                continue;                      //check if the word is smaller than given length, in that case go to while
            if (st.contains(key)) {            //check if the st contains the key, if key is in the symbol table
                st.put(key, st.get(key) + 1);  //in that case put it in the table with value +1
                words++;
            } else {
                st.put(key, 1);                //else put it with value 1
                distinct++;
            }
        }
        ...
        String max = "";
        st.put(max, 0);                       //put a value 0 to have something to compare with

        Stopwatch timer1 = new Stopwatch();   //A utility class to measure the running time (wall clock) of a program, start

        for (String word : st.keys()) {       //iterate all keys
            if (st.get(word) > st.get(max))   //check for the biggest value == most frecuency word
                max = word;
        }

        double time1 = timer1.elapsedTime();   //Stops the timer

        StdOut.println(max + " " + st.get(max));
        StdOut.printf("Runtime: " + "(%.4f seconds)\n", time1);
